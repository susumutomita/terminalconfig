...
```sh
brew install neovim ripgrep fd pngpaste
brew install --cask font-hack-nerd-font   # アイコン表示用。端末フォントに設定する
```

`pngpaste` は img-clip.nvim がクリップボードの画像を貼り付けるのに使う（macOS のみ必要）。
...
- img-clip.nvim — クリップボードの画像を貼り付け（保存してリンク挿入。macOS は pngpaste が必要）
- annai.nvim — `F1` / `Space ?` でやりたいことを聞くとキーマップを案内（自作・公開済み）
- vim-hybrid — 配色
...
| `F1` / `Space ?` | やりたいことを聞くとキーマップを案内（annai / 後述） |
| `Space e` | ファイルツリー開閉 |
...
| `Space V` | ライブプレビュー停止 |
| `Space p` | クリップボードの画像を貼り付け（macOS は pngpaste 必要） |
| `Ctrl-h` / `Ctrl-j` / `Ctrl-k` / `Ctrl-l` | ウィンドウ間移動（左 / 下 / 上 / 右） |
| `Esc Esc` | 検索ハイライト解除 |

保存（`:w`）すると、コードは Biome / Stylua で自動整形され、その他のファイルは末尾空白が自動削除される。

### やりたいこと案内（annai.nvim / `F1` ・ `Space ?`）

`F1`（または `Space ?`）で「やりたいこと」を自然言語で入力すると、ローカル LLM が現在の設定に実在するキーマップ（リーダー始まりの自作キー）だけから最も合う操作を選んで答える。回答は操作を邪魔しない窓に出る。この機能は自作プラグイン annai.nvim が提供する（ローカルに開発用クローンがあればそれを、無ければ GitHub の公開版を使う）。
...
2. `afm` が無い環境では Ollama にフォールバックする。
   ...
   ollama pull qwen2.5:3b   # init.lua の annai opts（ollama.model）で変更可
   ```
